...
v3 = x_hat;
%v3 = x;
v4 = x; %This is v3 for the centrlaize collaborative 

% scaled Lagrange Multipliers
d1  = 0*v1;
d2  = 0*v2;
d3  = 0*v3;
d4 = 0*v4; %D3 from centrlaized
...
while (i <= AL_iters) && ((abs (res_p) > tol1) || (abs (res_d) > tol2)) 
    ...
    v2 =  vector_soft_row(x_hat-d2,gamma/mu);
    ...
    v4 = vector_soft_row(x-d4,lambda/mu);
    ...
    if strcmp(addone,'yes')
       ...
       x = IF*(M'*(v1+d1)+(v2+d2)+(v3+d3)+(v4+d4));
    end
    ...
    if mod(i,10) == 1
        ...
        % primal residue
        res_p = sqrt(norm(M*x-v1,'fro')^2 + norm(x-v2,'fro')^2+ norm(x_hat-v3,'fro')^2+ norm(x-v4,'fro')^2);
        % dual residue
        %norm(v40,'fro')
        res_d = mu*norm(M'*(v1-v10)+v2-v20+v3-v30+v4-v40,'fro');
        if  strcmp(verbose,'yes')
            fprintf(' i = %f, res_p = %f, res_d = %f\n',i,res_p,res_d)
        end
        % update mu
%         if res_p > 1000000*res_d
%             mu = mu*2
%             d1 = d1/2;
%             d2 = d2/2;
%             d3 = d3/2;
